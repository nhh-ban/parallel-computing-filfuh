...
## Solving using parallel computing
# Set the maximum number of CPU cores
max_cpu_cores <- 2
num_cores <- min(parallel::detectCores(), max_cpu_cores)
...
# Load the 'tweedie' library on the cluster nodes
clusterEvalQ(cluster, library(tweedie))

# Run the parallel foreach loop to calculate 'share_reject'
